  ...
  <div id="gridContainer">
    <div class="gridItem one">
      <div v-for="(product, index) in products" :key="index">
          <div class="container"> 
          <div class="headline"><h1><router-link :to="{name: 'product', params: {id: product.product_id, thisproduct: product}}">{{ product.product_name }}</router-link></h1></div>
          <div class="image"><router-link :to="{name: 'product', params: {id: product.product_id, thisproduct: product}}"><img :src="'./src/assets/' + product.image_name" /></router-link></div>
...
export default {
  ...
  data() {
    return {
      products: []
      // productsData: Store.$data.products
    };
  },
  methods: {
    getProducts() {
